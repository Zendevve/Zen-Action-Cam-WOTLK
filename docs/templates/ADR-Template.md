...
- Environment: {{Local WoW 3.3.5a client / Private server}}
- Data and reset strategy: {{Fresh addon folder, clean WTF}}
- External dependencies: {{WoW client, private server}}

### Test commands

- build: `msbuild Extensions.sln /p:Configuration=Release`
- test: Manual in-game verification
- format: Lua style check

### New or changed tests

| ID | Scenario | Level (Unit / Int / Manual) | Expected result | Notes / Data |
| --- | --- | --- | --- | --- |
| {{TST-001}} | {{Happy path / negative / edge}} | {{Manual}} | {{Observable outcome}} | {{Steps to reproduce}} |

### Regression and analysis

- Regression suites to run: {{Manual test checklist}}
- Static analysis: {{luacheck, C++ warnings}}
- Monitoring during rollout: {{Console errors, FPS impact}}
